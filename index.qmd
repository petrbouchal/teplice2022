...
vko_r |> count(OBEC, ZKRATKAO8, wt = sum(podil_okrsku_na_strane)) |> 
  mutate(n = round(n)) |> 
  count(n)
...
percentily_all |> 
  mutate(podil_okrsku_na_strane = ptrr::label_percent_cz(accuracy = .1)(podil_okrsku_na_strane)) |> 
  select(ID_OKRSKY, OKRSEK, OBEC, obec_nazev, ZKRATKAO8, percentil, rank, podil_okrsku_na_strane) |> 
...
```{r percentily-teplice-overview}
percentily_teplice |> 
  count(ZKRATKAO8, wt = min(rank))
```

```{r percentily-all-plot}
percentily_all |> 
  drop_na(obec_nazev) |> 
  mutate(obec_nazev = as.factor(obec_nazev) |> fct_reorder(podil_okrsku_na_strane, max)) |> 
  ggplot(aes(podil_okrsku_na_strane, obec_nazev, colour = if_else(obec_nazev == "Teplice", "darkblue", "grey20"))) +
  ...
  geom_text(aes(label = if_else((podil_okrsku_na_strane > .05 & obec_nazev == "Teplice") | 
                                  (podil_okrsku_na_strane > .1 & obec_nazev != "Teplice"), ZKRATKAO8, NA_character_)), 
...
percentily_all  |> 
  drop_na(obec_nazev) |> 
  mutate(obec_nazev = as.factor(obec_nazev) |> fct_reorder(podil_okrsku_na_strane, mean)) |> 
...
bind_rows(simulace, simulace2) |> 
  ...
  geom_col() +
  geom_text(aes(label = round(value)), hjust = 0, nudge_x = 100, color = "white", fontface = "bold") +
  facet_wrap(~typ) +
  theme_ptrr(multiplot = TRUE, panel.spacing.x = unit(15, "pt")) +
...
```{r }
ptrr::ggsave_twitter("grafy-export/autokor.png", autokor_graf, scale = 2, height = 400, width = 900)
```
...
vko_r |> 
  ...
  facet_wrap(facets = c("ZKRATKAO8", "obec_nazev"), scales = "free_x") +
  geom_histogram(bins = 10) +
  theme(strip.text = element_text(lineheight = .8, size = 10, margin = margin(0)),
...
vko_r |> 
  ...
  summarise(skew = psych::skew(POC_HLASU),
            kurtosis = psych::kurtosi(POC_HLASU),
            normality_value = shapiro.test(POC_HLASU)[["statistic"]],
            normality_pvalue = shapiro.test(POC_HLASU)[["p.value"]] * 100) |> 
  arrange(desc(normality_value)) |> 
  pull(normality_pvalue) |> hist()
```
